...
public class UpdateFlightsBackgroundService : BackgroundService
{
    ...
    {
        public List<ProvidedFlight> providedFlights { get; set; }
    }
    ...
    public UpdateFlightsBackgroundService(ILogger<UpdateFlightsBackgroundService> logger, IHttpClientFactory clientFactory, ApiContext context)
    {
        _logger = logger;
            ...
            { 
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (flights.id != _lastProcessedId)
                    {
                        ...
                        foreach (var route in flights.legs.routes)
                        {
                            foreach (var flight in route.providers.providedFlights)
                            {
                        ...
                        await _context.SaveChangesAsync();
                        _logger.LogInformation("Successfully fetched flights from flights API, valid until {validUntil}", flights.validUntil);
                    }
                }
